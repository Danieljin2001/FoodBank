    ...
    $result = mysqli_query($con, $sql);
        if($result){
            while($row=mysqli_fetch_assoc($result)){
                ...
                //insert into f_supplies
                $sql1 = "insert into `F_supplies` (Forder_no, name) values ('$order_id', '$name')";
                if(mysqli_query($con, $sql1)){
                    $counter = 0;
                    $sql2 = "select * from `food` where name='$name' and Forder_no IS NULL";
                    $result1 = mysqli_query($con, $sql2);
                    while ($row1=mysqli_fetch_assoc($result1)){
                        if($counter < $qty){
                            $food_id = $row1['Food_id'];
                            $sql3="update `food` set Forder_no='$order_id' where Food_id='$food_id'";
